#!/usr/bin/env bash
# Apply docs/sql/phase9.sql to production Aurora — the estimate SyncToken baseline on `quotes`.
#
# Run it from the repo root:
#     bash docs/sql/apply-phase9.sh
#
...
#
# It needs the RDS master credentials because `quotes` is owned by `postgres`, not `app_user`.
# That was flagged as a blocking dependency in QBO-WEBHOOKS-PLAN.md §4 before the webhook work
# started — phase 1b and phase 2 hit the same wall. Do not go looking for a way around it: every
# apply script in docs/sql takes the master credentials and there is no exception to find.
#
...
# generated client knows about, so an image carrying qbo_sync_token / qbo_remote_changed_at that
# is deployed against a database without them fails EVERY read of `quotes` — the estimate list,
# opening an estimate, and the drain — with "column does not exist". Deploying first does not
# degrade the new feature, it takes the Estimator down.
#
# The success marker is PHASE9_APPLIED, derived from the SQL filename by build-runner.py, so a run
# cannot announce the wrong migration.
set -euo pipefail
...
[ "$EXIT_CODE" = "0" ] || exit 1
echo "Phase 9 applied. quotes.qbo_sync_token + qbo_remote_changed_at exist; deploy the drift image next."
