#!/usr/bin/env bash
# Apply docs/sql/phase4.sql to production Aurora.
#
...
#
# ORDER: this DDL must run BEFORE the image that reads it deploys. Prisma SELECTs every scalar
# column it knows about, so the moment the new client ships it asks for
# quotes.proposal_template_id and quotes.template_asked on every quote read — against an
# unmigrated DB that is a 500 on the core estimating path, not a degraded feature.
set -euo pipefail
...
[ "$EXIT_CODE" = "0" ] || exit 1
echo "Phase 3 applied. A customer name is now unique per parent, matching QuickBooks."
