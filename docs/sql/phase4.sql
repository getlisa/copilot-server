...
-- The drain's own query: oldest queued rows first.
CREATE INDEX IF NOT EXISTS "qbo_webhook_events_drain_idx"
    ON "public"."qbo_webhook_events"("status", "created_at");
...
BEGIN
  ...
  IF cols <> 16 THEN
    RAISE EXCEPTION 'qbo_webhook_events has % columns, expected 16', cols;
  END IF;
  ...
  RAISE NOTICE 'PHASE4_APPLIED: qbo_webhook_events, % columns, % indexes', cols, idx;
END $$;
