#!/usr/bin/env bash
# Apply docs/sql/phase3.sql to production Aurora.
#
...
#
# ORDER: safe either way. This only ever widens what the table accepts — it replaces a unique key
# with a strictly weaker one — so an image running against either version behaves correctly. It is
# grouped with the deploy that stops sending the narrower key.
set -euo pipefail
