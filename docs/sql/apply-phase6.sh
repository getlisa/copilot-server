#!/usr/bin/env bash
# Apply docs/sql/phase6.sql to production Aurora — the QuickBooks webhook event ledger.
#
# Run it from the repo root:
#     bash docs/sql/apply-phase6.sh
#
# Why this is a script and not "just run the SQL": Aurora is in a private VPC, so the statements
# have to execute from inside it — a one-off ECS task on the prod image is the only path. And it
# needs the RDS master credentials, which the service's own task definition does not carry.
#
# THAT LAST POINT WAS LEARNED THE HARD WAY, and the earlier version of this script asserted the
# opposite. The reasoning went: phase 6 only CREATEs its own new table, so it is owned by whoever
# runs it, so running as the service's own `app_user` is fine and no master secret is needed. That
# is wrong at the first statement — `app_user` holds no CREATE on schema `public` at all:
#
#     ERROR: permission denied for schema public   (SQLSTATE 42501)
#
# Creating a table is not a lesser privilege than altering one. Every apply script in docs/sql
# takes the master credentials, and there is no exception to find.
#
...
#
# ORDER: RUN THIS BEFORE DEPLOYING THE IMAGE THAT USES IT. Prisma SELECTs every scalar column its
# generated client knows about, so a webhook image deployed against a database without this table
# throws "relation does not exist" on every drain pass.
#
# NUMBERED 6, NOT 4. This work was written when main was at phase 3, but phases 5 and 5b landed
# and were applied while it was in review. A migration numbered below an already-applied one reads
# as "should have run first", which is the one thing a sequence number exists to say.
set -euo pipefail
...
[ "$EXIT_CODE" = "0" ] || exit 1
echo "Phase 6 applied. qbo_webhook_events exists; deploy the webhook image next."
