#!/usr/bin/env bash
# Apply docs/sql/phase5b.sql to production Aurora.
#
...
# generated client knows about, so the new image asks for company_configs.tax_enabled on every
# settings read the moment it starts. The reverse order 500s that screen until this lands.
set -euo pipefail
...
SQL_FILE="$(dirname "$0")/phase5b.sql"
WORK="$(mktemp -d)"
...
[ "$EXIT_CODE" = "0" ] || exit 1
echo "Phase 5b applied. Companies already charging tax keep charging it."
